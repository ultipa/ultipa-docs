...
- Graph model is as demonstrated by Chart1 in [Prepare Graph](/docs/quick-start/prepare-graph)
- Graph data can be downloaded from [Data Import](/docs/quick-start/data-import)
...
- The curly braces `{}` and its content held by `nodes()` (as well as `n()` and many other parameters, review [Graph Data - Describe Nodes](/docs/quick-start/graph-data)) is called <u>filtering condition</u>
...
Template query is an advanced type of graph query by accurately describing each node and edge in a path. It employs parameters `n()`, `e()` and `nf()` aforementioned in [Graph Data](/docs/quick-start/graph-data).
