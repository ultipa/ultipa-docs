...
Function `count()` calculates the number of rows in an alias (ignoring the rows that are <i>null</i>). Only one row from the alias will be kept after this function is called, and all the rest rows will be abandoned. 
...
Example: Calculate the number of all nodes in the graph
