...
Function `avg()` calculates the average row value in an alias (ignoring the rows that are <i>null</i>). Only one row from the alias will be kept after this function is called, and all the rest rows will be abandoned. 

Arguments：
- Alias \<number | time>

Returns：
- Avg value \<number | time>
...
Example: Calculate the average <i>age</i> of all nodes in the graph
