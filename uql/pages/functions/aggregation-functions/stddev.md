...
Function `stddev()` calculates the standard deviation of numbers in an alias (ignoring the rows that are <i>null</i>). Only one row from the alias will be kept after this function is called, and all the rest rows will be abandoned. 
...
Example: Calculate the standard deviation of <i>age</i> of all nodes in the graph
