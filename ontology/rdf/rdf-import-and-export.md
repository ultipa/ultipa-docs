...
- **IRI schemes.** An IRI of any scheme is kept as an absolute IRI on both load and export: `http`/`https` and `urn`, as well as non-network schemes such as `mailto:`, `tel:`, `file:`, and `did:`. Only a genuine relative reference (a fragment, a relative path, or a network-path reference) is resolved against the document's base. So a value like `<mailto:alice@example.org>` round-trips unchanged rather than being concatenated onto the file base.
