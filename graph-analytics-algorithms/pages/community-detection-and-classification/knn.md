...
Ultipa's kNN algorithm computes pair-wise <a href="https://www.ultipa.com/document/ultipa-graph-analytics-algorithms/cosine-similarity">cosine similarity</a> between the target node and all other nodes in the graph, then selects the <i>k</i> nodes with the highest similarity to the target node.
...
| node_schema_property | []`@<schema>?.<property>` | Numeric type, must LTE, must be of the same schema as the target node | / | No | Two or more node properties to compute the <a href="https://www.ultipa.com/document/ultipa-graph-analytics-algorithms/cosine-similarity">cosine similarity</a> |
