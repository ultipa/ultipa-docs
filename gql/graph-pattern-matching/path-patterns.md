...
> **Default is `TRAIL`, not `WALK`.** This is a deliberate deviation from ISO/IEC 39075, where `WALK` is the default. Edge-unique paths are what most queries want, so a plain `MATCH` never traverses the same edge twice; edge uniqueness applies to the whole path (an edge used in one segment cannot be reused in a following quantified segment). Specify `WALK` explicitly when repeated edges are wanted.
